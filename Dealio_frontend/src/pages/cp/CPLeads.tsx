              ...
              return (
                <div key={stage} className="min-w-[230px] flex-shrink-0">
                  ...
                  <div className="space-y-2">
                    ...
                    {stageLeads.map(lead => (
                      <div key={lead.id} className="bg-white rounded-xl p-3 border border-slate-100 shadow-sm text-xs hover:border-slate-200 hover:shadow-md transition-all">
                        ...
                        <div className="flex items-start justify-between gap-1 mb-1">
                          ...
                          <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full shrink-0 ${style.bg} ${style.text}`}>
                            {stage}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-slate-50">
                          <div className="flex gap-1">
                            ...
                            <button
                              onClick={() => { setMeetingLead(lead); setMeetingDate(''); setMeetingTime(''); }}
            ...
            <div className="p-5 space-y-4">
              <div>
                <DatePickerField
                  ...
                  value={meetingDate}
                  onChange={v => { setMeetingDate(v); setMeetingTime(''); }}
              ...
              {meetingDate && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {TIME_SLOTS.map(slot => (
                      <button key={slot} onClick={() => setMeetingTime(slot)}
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${meetingTime === slot ? 'bg-teal-600 text-white border-teal-600' : 'bg-gray-50 text-gray-600 border-gray-200 hover:border-teal-300'}`}>
                        {slot}
                      </button>
                    ))}
                  </div>
