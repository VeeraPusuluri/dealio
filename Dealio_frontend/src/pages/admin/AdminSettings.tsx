...
import { User, Mail, Phone, Moon, Sun, Shield } from 'lucide-react';

const AdminSettings = () => {
  const authUser = useAuthStore((s) => s.user);
  const { isDark, toggle: toggleTheme } = useThemeStore();

  const color = authUser ? roleColors[authUser.role] || '#6B3FA0' : '#6B3FA0';
        ...
        <div className="rounded-2xl border border-border overflow-hidden bg-card">
          ...
          <div className="px-5 pb-5 -mt-7 flex items-end gap-4">
            ...
            <div className="pb-0.5 flex-1 min-w-0">
              <h2 className="text-[15px] font-bold text-card-foreground truncate">{authUser?.name || 'Admin'}</h2>
              <div className="flex items-center gap-2 mt-1">
                <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full text-white" style={{ backgroundColor: color }}>
                  {authUser ? roleLabels[authUser.role] : 'Admin'}
                </span>
                <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-muted-foreground">
                  <Shield size={10} /> Platform administrator
                </span>
              </div>
            </div>
          </div>
