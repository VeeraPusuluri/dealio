...
const LoginPage = () => {
  ...
  const initialMode: Mode = params.get('tab') === 'signup' ? 'signup' : 'signin';
  ...
  const handleSendPhoneOtp = async (e: React.FormEvent) => {
    ...
    const phErr = !phone.trim() ? 'Phone number required' : phone.replace(/\D/g, '').length < 7 ? 'Enter a valid phone' : '';
  ...
  const submitBtnCls   = 'w-full py-3.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 text-white transition-all hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-50 disabled:hover:translate-y-0 shadow-md shadow-[#0A7E8C]/25';
    ...
    <div className="min-h-screen flex flex-col lg:flex-row">
        ...
        <div className="relative z-10 flex flex-col h-full p-10 xl:p-14">
          ...
          <div className="flex-1 flex flex-col justify-center mt-10">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/8 border border-white/15 text-white/75 text-xs font-semibold w-fit mb-6">
              ...
              {mode === 'signup' ? 'Join 12,400+ stakeholders' : 'Trusted by 12,400+ stakeholders'}
            </div>

            <h2 className="text-3xl xl:text-[38px] font-black text-white leading-tight tracking-tight mb-3">
              {mode === 'signin' ? <>Welcome back<br /></> : <>Start your journey<br /></>}
              <span style={{ background: 'linear-gradient(90deg,#FCD34D,#F59E0B,#E87722)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                {mode === 'signin' ? 'to Dealio' : 'on Dealio'}
              </span>
            </h2>

            <p className="text-white/60 text-sm xl:text-[15px] mb-10 leading-relaxed max-w-sm">
              {mode === 'signin'
                ? 'Sign in to your role-specific dashboard and pick up where you left off.'
      ...
      <div className="flex-1 flex flex-col min-h-screen lg:min-h-0 lg:overflow-y-auto bg-white">
        ...
        <div className="lg:hidden flex items-center justify-between px-5 py-4 border-b border-slate-100">
          ...
          {mode === 'signin'
            ? <button onClick={() => switchMode('signup')} className="text-sm font-semibold text-[#0A7E8C]">Sign up</button>
            : <button onClick={() => switchMode('signin')} className="text-sm font-semibold text-[#0A7E8C]">Sign in</button>
          }
        </div>
        ...
        <div className="flex-1 flex items-start lg:items-center justify-center px-5 sm:px-8 py-8 lg:py-12">
          <div className="w-full max-w-md">
            ...
            <>
              {/* Header */}
              <div className="mb-6">
                <h1 className="text-2xl sm:text-[28px] font-black text-[#0F2035] tracking-tight">
                  {mode === 'signin' ? 'Welcome back' : 'Create your account'}
                </h1>
                <p className="text-slate-500 text-sm mt-1.5">
                  {mode === 'signin' ? (
                    <>New to Dealio?{' '}
                      <button onClick={() => switchMode('signup')} className="font-semibold text-[#0A7E8C] hover:underline">Sign up free</button>
                    </>
                  ) : (
                    <>Already have an account?{' '}
                      <button onClick={() => switchMode('signin')} className="font-semibold text-[#0A7E8C] hover:underline">Sign in</button>
                    </>
                  )}
                </p>
              </div>
              ...
              <button type="button" disabled={busy}
                ...
                className="w-full mb-6 py-2.5 rounded-xl text-xs font-semibold text-slate-500 hover:text-[#0F2035] border border-dashed border-slate-200 hover:border-slate-300 hover:bg-slate-50 transition-all">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <input type="tel" inputMode="numeric" required placeholder="98765 43210" value={phone}
                        onChange={(e) => { setPhone(e.target.value.replace(/[^\d\s]/g, '')); if (phoneErr) setPhoneErr(''); }}
              ...
              {method === 'phone' && phoneOtpSent && (
                <form onSubmit={handleVerifyPhoneOtp} className="space-y-4">
                  <div className="bg-[#0A7E8C]/6 border border-[#0A7E8C]/20 rounded-xl p-4">
                    ...
                    <p className="text-xs text-slate-400 mt-1">Expires in 5 minutes</p>
                  </div>
                  <div>
